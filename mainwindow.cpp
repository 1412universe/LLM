    ...
    audioRecorder(new QAudioRecorder(this))
{
    ...
    // 添加录音设置
     ...
     QNetworkAccessManager *managerVoice = new QNetworkAccessManager;
     ...
     // 配置网络请求
     QNetworkRequest request(QUrl("http://127.0.0.1:8080/api/chat/start_detection"));
     request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");

     // 发送 POST 请求
     QNetworkReply *reply = managerVoice->post(request, QJsonDocument(jsonData).toJson());
     ...
     QObject::connect(reply, &QNetworkReply::finished, [=]() {
         ...
         if (jsonResponse.contains("status") && jsonResponse["status"].toString() == "success") {
             ui->ProcessBrowser->append("响应: " + jsonResponse["response"].toString());
         } else {
             QString errorMsg = jsonResponse.contains("response")
                 ? jsonResponse["response"].toString()
                 : "未知错误";
             ui->ProcessBrowser->append("错误: " + errorMsg);
         }
         reply->deleteLater();
         managerVoice->deleteLater();
     });
...
// 在MainWindow类中添加HTML转Markdown的辅助函数
QString MainWindow::htmlToMarkdown(const QString& html)
{
    QString md = html;
    ...
    // 移除div和class属性
    QRegularExpression divRe("<div[^>]*>");
    md.remove(divRe);
    md.replace("</div>", "");

    // 转换链接 [text](url)
    QRegularExpression linkRe("<a\\s+href='([^']+)'[^>]*>(.*?)</a>");
    md.replace(linkRe, "[\\2](\\1)");

    // 转换图片 ![alt](src)
    QRegularExpression imgRe("<img\\s+[^>]*src='([^']+)'[^>]*>");
    md.replace(imgRe, "![image](\\1)");
...
void MainWindow::on_SaveBtn_clicked()
{
    ...
    // 获取并转换内容
    QString html = ui->ProcessBrowser->toHtml();
